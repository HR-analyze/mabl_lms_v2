...
/**
 * Каркас кабинета: боковая навигация + верхняя панель.
 * Единый «шелл» для всех страниц авторизованного пользователя — чтобы при
 * переходах по разделам сайдбар всегда оставался на месте.
 */
export function AppShell({ children, surfaceClassName = 'bg-ink-5' }: {
  children: ReactNode
  /** Фон рабочей области (кабинет — серый, контентные страницы — белый). */
  ...
  return (
    <div className={cn('min-h-screen', surfaceClassName)}>
      {/* Десктоп-сайдбар */}
