...
- **Реальные платежи (PSP)** — оплата сейчас имитируется на сервере, но **заказ и
  доступ к программе создаются по-настоящему** (таблицы `orders`, `enrollments`).
  Следующий шаг: интеграция ЮKassa/Stripe (создание платежа + webhook
  подтверждения), вместо имитации в `/me/courses/:id/purchase`.
